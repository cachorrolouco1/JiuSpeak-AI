...
import { Video, Users, PhoneCall, X, RefreshCw } from 'lucide-react';
        ...
        <div className="p-5 space-y-3">
          ...
          {loading ? (
            ...
            <div className="text-center py-4">
              <p className="text-xs" style={{ color: 'var(--js-text-muted)' }}>Nenhuma chamada em grupo ativa no momento</p>
            </div>
          )}
          ...
          >
            ...
            <div className="text-left">
              <p className="text-sm font-bold" style={{ color: 'var(--js-text-primary)' }}>Chamada Individual</p>
              <p className="text-[10px]" style={{ color: 'var(--js-text-muted)' }}>Aula particular com o professor</p>
            </div>
          </button>

          <button onClick={() => onJoinGroup('jiuspeak-group-' + Date.now())}
            className="w-full p-4 rounded-2xl flex items-center space-x-4 transition-all active:scale-95"
            style={{ backgroundColor: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)' }}
          >
            <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f59e0b, #8b5cf6)' }}>
              <Users className="w-5 h-5 text-white" />
            </div>
            <div className="text-left">
              <p className="text-sm font-bold" style={{ color: 'var(--js-text-primary)' }}>Criar Sala em Grupo</p>
              <p className="text-[10px]" style={{ color: 'var(--js-text-muted)' }}>Outros alunos podem entrar</p>
            </div>
          </button>
        </div>
      </div>
